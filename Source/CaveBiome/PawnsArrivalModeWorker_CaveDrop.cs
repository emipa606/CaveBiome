...
namespace CaveBiome
{
    ...
    public class PawnsArrivalModeWorker_CaveDrop : PawnsArrivalModeWorker
    {
        ...
        private IntVec3 FindAGoodSpot(Map map)
        {
            ...
            if ((from caveWell in caveWellsList where DropCellFinder.TryFindDropSpotNear(caveWell.Position, map, out _, false, false, true, null) select caveWell.Position).TryRandomElement(out IntVec3 dropPlace))
            {
                return dropPlace;
            }
            return IntVec3.Invalid;
